...
async def _async_collect(hass: HomeAssistant, entry: SihasConfigEntry,
    ...
    private_values = [item.get("attributes", {}) for item in result["entities"]["items"]]
    ...
    for index, item in enumerate(result["entities"]["items"]):
        ...
        if "attributes" in item:
            attributes = export.entity_attributes(item["attributes"], path + ".attributes")
            ...
            for key in {"attribution", "options", "fan_mode", "fan_modes", "preset_mode", "preset_modes"} & attributes.keys():
